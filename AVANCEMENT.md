...
| Progression | Sélecteur groupé par muscle (chips + dropdown), graphique fl_chart : poids max / volume / 1RM estimé — ou durée max / durée totale pour les exercices en durée et cardio. Tuiles record / 1RM / dernier volume. **Seuls les exercices ayant au moins une série enregistrée apparaissent dans les filtres** (provider `exercisesWithDataProvider`), idem sur l'écran Comparaison |
